RSpec.describe 'All Pattern' do
  ...
  it 'not all zeros' do
    ...
    numbers.each do |number|
      all_zeros = false unless number.zero?
      # this reads as:
      # all_zeros = false if number does not equal zero
      # so all_zeros will remain true if line 15 never hits
      # a number that does not equal zero
    end
