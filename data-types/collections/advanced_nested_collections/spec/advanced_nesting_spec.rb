...
RSpec.describe 'Advanced Nested Collections' do
  ...
  it 'test 7' do
    ...
    # all restaurants
    employee_names = []
    stores.keys.each { |store| employee_names.concat(stores[store][:employees]) }
  ...
  it 'test 8' do
    ...
    ingredients = []
    stores.keys.each { |store| stores[store][:dishes].each { |dish| ingredients.concat(dish[:ingredients]) } }
  ...
  it 'test 11' do
    ...
    stores.keys.each { |store| stores[store][:dishes].each { |dish| full_menu[dish[:name]] = dish} }
